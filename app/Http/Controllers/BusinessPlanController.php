...
class BusinessPlanController extends Controller
{
    public function create() {
        ...
        $user = BusinessPlan::where('email', Auth::user()->email)->first();
        $count_user = BusinessPlan::all()->count();
        $competition = ListCompetition::where('competition', 'BusinessPlan')->first();
        ...
        if ($user) {
            return back()->with("error", "You have registered for the Business Plan competition");   
        }
    ...
    public function registration (Request $request) {
        ...
        if($request->file('idcard1') && $request->file('idcard2') && $request->file('payment')){
            ...
            $BusinessPlan = new BusinessPlan($validation);
    ...
    public function submission(Request $request) {
        ...
        if ($user) {
            ...
            if ($request->file('submission')) {
                ...
                if ($user->submission) {
                    $old_path = public_path('BusinessPlan\\').$user->submission;
                ...
                $submission->move(public_path('BusinessPlan'), $filesubmission);
